...
func (s *PostgresStore) QueueRecordingFinalizationJobsForInactiveMissions(ctx context.Context) (int64, error) {
	runner := s.sqlRunner()
	var queued int64
...
func (s *PostgresStore) ClaimRecordingFinalizationJobs(ctx context.Context, workerID string, limit int, lockDuration time.Duration) ([]domain.RecordingFinalizationJob, error) {
	if limit <= 0 || limit > 50 {
		limit = 10
	}
	if lockDuration <= 0 {
...
func (s *PostgresStore) MarkRecordingFinalizationJobCompleted(ctx context.Context, jobID string, workerID string, attempt int) error {
	return s.markRecordingFinalizationJob(ctx, jobID, "completed", workerID, attempt, "")
}

func (s *PostgresStore) MarkRecordingFinalizationJobPartial(ctx context.Context, jobID string, workerID string, attempt int, reason string) error {
	return s.markRecordingFinalizationJob(ctx, jobID, "partial", workerID, attempt, reason)
}

func (s *PostgresStore) MarkRecordingFinalizationJobFailed(ctx context.Context, jobID string, workerID string, attempt int, reason string) error {
	return s.markRecordingFinalizationJob(ctx, jobID, "failed", workerID, attempt, reason)
}

func (s *PostgresStore) markRecordingFinalizationJob(ctx context.Context, jobID string, status string, workerID string, attempt int, reason string) error {
	runner := s.sqlRunner()
	var chunkID string
...
func (s *PostgresStore) validateRecordingFinalizationUploadClaim(ctx context.Context, runner sqlContextRunner, chunkID string, uploadMetadata RecordingUploadMetadata) error {
	var status string
	var lockedBy string
	var attempts int
	err := runner.QueryRowContext(ctx, `
...
func (s *PostgresStore) refreshRecordingSessionStatusForChunk(ctx context.Context, runner sqlContextRunner, chunkID string) error {
