...
func (s *Store) sqlRunner() sqlContextRunner {
	if s.sqlTx != nil {
		return s.sqlTx
	}
	return s.sqlDB
...
func (s *Store) FindRecordingTarget(ctx context.Context, missionCode string, robotCode string) (repo.RecordingTarget, error) {
	mission, robotID, err := s.findMissionRobotForRecording(ctx, s.sqlRunner(), missionCode, robotCode)
	if err != nil {
		return repo.RecordingTarget{}, err
	}
	return repo.RecordingTarget{
		Mission:   mission,
		RobotID:   robotID,
...
func (s *Store) FindOrCreateRecordingSession(ctx context.Context, missionID string, robotID string, chunkDurationSeconds int, startedAt time.Time) (repo.RecordingSession, error) {
	return s.findOrCreateRecordingSession(ctx, s.sqlRunner(), missionID, robotID, chunkDurationSeconds, startedAt)
}

func (s *Store) FindRecordingChunk(ctx context.Context, recordingSessionID string, chunkIndex int) (domain.RecordingChunk, bool, error) {
	return s.findRecordingChunk(ctx, s.sqlRunner(), recordingSessionID, chunkIndex)
}

func (s *Store) CreateRecordingChunk(ctx context.Context, input repo.CreateRecordingChunkInput) (domain.RecordingChunk, error) {
...
func (s *Store) MarkRecordingChunkUploaded(ctx context.Context, chunkID string, uploadMetadata repo.RecordingUploadMetadata) (domain.RecordingChunk, error) {
	runner := s.sqlRunner()
...
func (s *Store) MarkRecordingFileUploaded(ctx context.Context, chunkID string, fileType string, uploadMetadata repo.RecordingUploadMetadata) (domain.RecordingChunk, error) {
	runner := s.sqlRunner()
	if err := s.validateRecordingFinalizationUploadClaim(ctx, runner, chunkID, uploadMetadata); err != nil {
		return domain.RecordingChunk{}, err
	}
...
func (s *Store) upsertRecordingStorageObject(ctx context.Context, runner sqlContextRunner, chunk domain.RecordingChunk, fileType string, uploadMetadata repo.RecordingUploadMetadata) (string, error) {
...
func (s *Store) findMissionRobotForRecording(ctx context.Context, runner sqlContextRunner, missionCode string, robotCode string) (domain.Mission, string, error) {
...
func (s *Store) getRecordingChunkByID(ctx context.Context, runner sqlContextRunner, chunkID string) (domain.RecordingChunk, error) {
	row := runner.QueryRowContext(ctx, recordingChunkSelectSQL()+`
		WHERE rc.id = $1::uuid
	`, strings.TrimSpace(chunkID))
	chunk, err := scanRecordingChunk(row)
	if errors.Is(err, sql.ErrNoRows) {
		return domain.RecordingChunk{}, repo.ErrNotFound
	}
	return chunk, err
}
