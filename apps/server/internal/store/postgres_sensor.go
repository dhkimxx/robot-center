...
func (s *PostgresStore) SaveSensorEnvelope(ctx context.Context, envelope domain.SensorEnvelope) ([]domain.SensorSample, error) {
	...
	if len(envelope.Descriptors) > 0 {
		records := make([]sensorDescriptorRecord, 0, len(envelope.Descriptors))
		for _, descriptor := range envelope.Descriptors {
			...
			channelRole := firstNonEmpty(descriptor.ChannelRole, envelope.ChannelRole, "channel.telemetry")
			metadata := descriptor.Metadata
			if len(metadata) == 0 {
				metadata = []byte("{}")
			}
			displayName := firstNonEmpty(descriptor.DisplayName, descriptor.SensorID)
			sensorType := firstNonEmpty(descriptor.SensorType, "unknown")
			valueType := firstNonEmpty(descriptor.ValueType, "object")
	...
	for _, sample := range envelope.Samples {
		...
		records = append(records, sensorSampleRecord{
			DescriptorID: descriptorID,
			MissionID:    envelope.MissionID,
			RobotID:      robotID,
			SensorID:     sample.SensorID,
			ChannelRole:  firstNonEmpty(sample.ChannelRole, envelope.ChannelRole, "channel.telemetry"),
			MessageID:    optionalString(firstNonEmpty(sample.MessageID, envelope.MessageID)),
			Sequence:     optionalInt64(firstNonZeroInt64(sample.Sequence, envelope.Sequence)),
			SentAt:       firstTimePointer(sample.SentAt, envelope.SentAt),
...
func (s *PostgresStore) ListSensorDescriptors(ctx context.Context, missionID string, robotCode string) ([]domain.SensorDescriptor, error) {
...
func (s *PostgresStore) ListSensorSamples(ctx context.Context, missionID string, robotCode string, sensorID string, limit int) ([]domain.SensorSample, error) {
	missionID, err := s.resolveMissionID(ctx, missionID)
	if err != nil {
		return nil, err
	}
...
func (s *PostgresStore) ListLatestSensorSamples(ctx context.Context, missionID string, robotCode string) ([]domain.SensorLatest, error) {
	missionID, err := s.resolveMissionID(ctx, missionID)
	if err != nil {
		return nil, err
	}
...
func (s *PostgresStore) resolveMissionID(ctx context.Context, missionIDOrCode string) (string, error) {
