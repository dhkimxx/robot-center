...
녹화 리플레이 화면, 녹화 파일 URL, playback modal에 영향을 주는 변경은 replay smoke 확인을 추가한다.

```bash
./scripts/deploy-verify.sh --no-commit --replay-smoke --replay-smoke-mission mission-054 --replay-smoke-robot robot-042
```

개발서버 재시작 뒤 mock publisher까지 하네스가 보장해야 하면 full smoke를 사용한다.

```bash
./scripts/deploy-verify.sh --no-commit --ensure-python-mock --full-smoke --smoke-mission mission-054 --smoke-robot robot-042
```

`--ensure-python-mock`은 `scripts/deploy-ensure-python-mock.sh`로 지정한 개발서버 Python mock을 재시작하고 `scripts/deploy-verify-smoke-readiness.py`로 readiness를 확인하므로, 로봇팀 실제 송출 테스트 중에는 명시적으로 필요한 경우에만 사용한다.
...
- 하네스가 커버하지 못하는 Mock Robot 장시간 구동, 특수 센서, 녹화 파일 장시간 재생 검증은 작업 성격에 맞게 별도로 수행한다.
