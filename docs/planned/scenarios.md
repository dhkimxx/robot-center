...
- Frontend Dashboard: 임무 현황, 로봇 상태, 영상, 센서, 탐지 결과, 알람, SOP 제안을 표시한다.
- Backend API Server: 인증, 권한, 임무, 이벤트, 탐지 결과, 로봇 제어 API를 제공한다.
- SFU / Media Gateway: Robot stream을 한 번 받아 Browser와 Recorder Peer로 분배한다.
- Recorder Peer: SFU subscriber로 붙어 영상, 센서, 탐지 데이터를 수신하고 PostgreSQL/MinIO 저장을 담당한다.
- Realtime Gateway: 로봇 상태, 센서 데이터, 탐지 이벤트, 알람을 실시간 전달한다.
- Video Streaming: RGB/Thermal 영상 표시와 이벤트 시점 북마크를 지원한다.
- Sensor Ingestion: 가스, 온도, 습도, IMU, LiDAR 관련 데이터를 수신하고 저장한다.
- Detection Service: 인명 탐지, 열원 탐지, 가스 이상치 등 탐지 결과를 이벤트로 구조화한다.
- LLM/SOP Agent: 상황 요약, 위험도 분석, SOP 매핑, 대응 시나리오 추천을 수행한다.
- Database/Storage: 임무, 로봇 상태, 센서 시계열, 이벤트, 탐지 결과, 영상 메타데이터를 저장한다.

### 현재 연동 합의 상태

Robot팀과 AI Web팀 사이의 구체적인 데이터 연동 인터페이스는 아직 확정되지 않았다.

현재 확정된 전제는 WebRTC 기반 연동이다. 따라서 본 문서의 연동 항목은 최종 계약이 아니라, 향후 Robot팀과 협의해야 할 요구사항 후보로 본다.

WebRTC 기반 연동은 다음 범위로 구분해 설계한다.

- Media Track: RGB 영상, Thermal 영상, 음성 스트림
- DataChannel: 로봇 상태, 센서 데이터, 탐지 이벤트, 제어 명령, ACK/NACK
- Signaling: 세션 생성, 로봇 접속, SDP 교환, ICE Candidate 교환, 재연결

권장 WebRTC 참여 주체는 다음 4개 역할이다.

- Robot Peer: Jetson/로봇 측 WebRTC peer. RGB/Thermal/Audio Media Track과 telemetry/sensor/detection DataChannel을 송신하고 command DataChannel을 수신한다.
- SFU / Media Gateway: Robot stream을 한 번 받아 Browser와 Recorder Peer로 분배한다.
- Recorder Peer: SFU subscriber로 붙어 영상, 센서, 탐지 데이터를 수신하고 PostgreSQL/MinIO 저장을 담당한다.
- Browser Peer: 관제 화면의 WebRTC peer. 실시간 영상/음성/상태를 수신하고 관제 UI 이벤트를 서버로 전달한다.
...
- LLM/SOP Agent와 Detection Service가 실시간 데이터를 서버 내부에서 사용할 수 있어야 한다.
...
시나리오가 추가되면 `docs/README.md`의 P0 기준, 아키텍처, API 계약 문서에서 다음 항목을 함께 갱신한다.
...
WebRTC 기반 연동을 전제로 하되, 다음 항목은 Robot팀과 별도 협의가 필요하다.

- WebRTC 연결 주체: Robot이 offer를 생성할지, AI Web 서버가 offer를 생성할지
- SFU signaling 세부 절차: room join, publish, subscribe, ICE restart 메시지 형식
- Recorder Peer 저장 파이프라인 세부 구현: snapshot, chunk, MP4 muxing 단계
- Signaling 방식: REST, WebSocket, 또는 별도 signaling server 사용 여부
- ICE 구성: 별도 STUN/TURN 애플리케이션 주소, 인증 방식, 폐쇄망 실증 환경 구성
- Media Track 구성: RGB, Thermal, Audio를 개별 Track으로 보낼지, 하나의 합성 영상으로 보낼지
- DataChannel 이름과 용도: telemetry, sensor, detection, command, controlAck 등 채널 분리 여부
- DataChannel 신뢰성 옵션: ordered, unordered, maxRetransmits 설정
- 메시지 공통 필드: messageId, robotId, missionId, timestamp, sequence, schemaVersion
...
- 보안: WebRTC DTLS/SRTP 외 인증, 권한, 세션 토큰, 로봇 식별 방식
