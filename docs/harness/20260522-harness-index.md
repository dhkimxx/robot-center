...
| `docs/harness/20260523-multi-robot-sfu-checklist.md` | mission 단위 multi-robot SFU 검증 체크리스트 |

## 확정 범위

- Robot publisher들이 같은 `missionCode` room에 WebRTC publish 한다.
- Browser subscriber A/B가 같은 mission room에서 live 관제용으로 subscribe 한다.
- Recorder subscriber가 같은 mission room에서 저장용으로 subscribe 한다.
