...
type: "feature"
status: "in-progress"
tags: ["feature", "i_com", "i-com-03", "in-progress"]
history:
...
2026-06-08 기준으로 robot reconnect 이후 관제 subscriber 복구 흐름이 보강되었다.

반영된 범위:

- robot publisher reconnect 이후 operator subscriber가 다시 track을 받을 수 있도록 복구 로직 개선
- SFU subscriber session 테스트 보강
- WebRTC candidate, publisher/subscriber 주요 흐름에 focused monitoring log 추가
- SDP monitoring과 legacy SCTP DataChannel 감지 로직 추가

아직 완료가 아닌 이유:

- 5G/WiFi/LTE 자동 채널 전환은 관제 서버 범위에서 구현되지 않았다.
- Edge 로컬 모드, Delta Sync, 최저대역폭 모드는 아직 설계/구현 대상이다.
- 현재 범위는 WebRTC room 내부 재연결과 관제 화면 복구 안정화에 한정된다.
