...
type: "feature"
status: "in-progress"
tags: ["feature", "e_db", "e-db-03", "in-progress"]
history:
...
SensorDescriptor/SensorSample 구조, latest sensor API, telemetry/spatial DataChannel 저장

## 원본 XLSX 상세 설명

TimescaleDB 기반 시계열 데이터 저장(가스농도/온도/습도/IMU/LiDAR), 고속 쓰기 최적화(배치 인서트), 자동 압축/집계(오래된 데이터), 시계열 쿼리 최적화.
