...
- '2026-06-08 danya.kim <danya.kim@thundersoft.com>: clarify channel.event append-only storage and live projection split'
- '2026-06-08 danya.kim <danya.kim@thundersoft.com>: align event storage columns with values-based event contract'
---
...
| `channel.event` | `events` append-only 로그. Live overlay는 별도 최신 snapshot projection으로 처리 | JSONL 저장 안 함 |
| `channel.control` | 현재 저장 대상 아님 | TODO |

Python mock은 `channel.control`을 열지만 payload를 자동 송신하지 않는다. `channel.event`는 Live UI event v0 검증 시나리오에서만 payload를 송신한다.
...
| `mission_id` | uuid | yes | FK missions.id |
| `robot_id` | uuid | no | FK robots.id |
| `event_id` | text | no | 로봇 또는 추론 모듈이 생성한 event id |
| `event_type` | text | yes | event type |
| `event_category` | text | yes | mission, detection, alarm, system 등 조회용 category |
| `track_id` | text | no | detection overlay 대상 track. 예: `track.video_1` |
| `severity` | text | yes | info, warning, critical 등 |
| `title` | text | yes |  |
| `description` | text | no |  |
| `occurred_at` | timestamptz | yes | mission timeline 기준 |
| `received_at` | timestamptz | yes | 관제 서버 수신 시각 |
| `detection_count` | integer | no | `detection.object` snapshot의 detection 개수. empty clear snapshot은 `0` |
| `geom` | geometry(Point,4326) | no | 이벤트 위치 |
| `related_storage_object_id` | uuid | no | FK storage_objects.id |
| `values` | jsonb | yes | eventType별 values |
| `raw_message` | jsonb | yes | DataChannel event envelope 원문 |
| `raw_payload` | jsonb | yes | default `{}` |
| `created_at` | timestamptz | yes | default now |

조회 원칙:

- `events`는 append-only 원본 이벤트 로그다.
- 일반 mission event feed는 기본적으로 `detection.object`를 제외한다.
- 객체 탐지 이력은 `event_type = 'detection.object'`와 `track_id` 기준으로 조회한다.
- Live overlay는 이 테이블을 polling하지 않고, WebRTC DataChannel에서 받은 최신 snapshot projection을 사용한다.
- ACK, 처리 상태, operator 확인 이력은 후속 범위에서 별도 테이블로 둔다.
