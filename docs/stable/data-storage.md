...
| Runtime Session | `robot_sessions`, `browser_sessions`, `recorder_sessions`, `streaming_statuses` | heartbeat/session/status 추적. 일부 session table은 향후 활용 여지 포함 |
...
`robots.status`는 장치 상태만 표현한다. 임무 배정 여부는 `mission_robots`, WebRTC 송출 여부와 freshness는 `streaming_statuses`를 기준으로 판단한다.
...
Robot Gateway의 streaming status 보고를 robot별 최신 상태로 저장한다.
...
현재 streaming freshness window는 30초 기준이며, 임무 생성/시작 차단과 UI fresh 판단은 `updated_at`을 기준으로 한다. robot clock skew 때문에 `sent_at`만으로 fresh 여부를 판단하지 않는다.
...
현재 `FindOrCreateRecordingSession`은 `mission_id + robot_id + ended_at IS NULL` 기준으로 최신 session을 재사용한다.

### 9.2 recording_chunks

`recording_chunks`는 10분 기본 window 기준 chunk lifecycle과 object 연결을 관리한다.
...
5. streaming_statuses에 robot별 room/status/published tracks 기록
