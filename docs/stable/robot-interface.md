...
| `sensorType` | string | No | 없으면 sensorId에서 추론 |
| `valueType` | string | No | 없으면 `object` |
| `unit` | string | No | 표시 단위 |
| `sampleRateHz` | number | No | sampling rate |
...
| `values` | any | No | 실제 측정값. `valueType`에 맞는 JSON number/string/boolean/object/vector 형태 |
