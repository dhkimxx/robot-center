...
- operator/admin REST API
- robot-scoped REST API `/api/v1/robot/*`
- robot 등록/조회/수정
- robot token 발급과 검증
- heartbeat 수신
- mission 생성/시작/종료
...
sequenceDiagram
  ...
  Robot->>App: /api/v1/robot/heartbeat + Bearer robotToken
  App->>DB: robot 상태 갱신

  Robot->>App: /api/v1/robot/mission + Bearer robotToken
  App-->>Robot: own active missionCode, signalingUrl, TURN info

  Robot->>SFU: /api/v1/robot/sfu/ws?room=missionCode + Bearer robotToken
